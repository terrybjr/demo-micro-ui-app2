pipeline {
    ...
    stages {
        ...
        stage('Stop Existing Docker Container') { // Stage for stopping any existing Docker container
            steps {
                script {
                    // Stop any running Docker containers from the 'demo-micro-ui-app2:latest' image
                    sh '''
                       docker ps -q -f ancestor=demo-micro-ui-app2:latest | xargs -r docker stop
                       docker ps -a -q -f ancestor=demo-micro-ui-app2:latest | xargs -r docker rm
                    '''
                }
            }
        }
        stage('Build Docker Image') { // Stage for building the Docker image
            steps {
                script {
                    docker.build('demo-micro-ui-app2:latest', './web-app') // Build Docker image from the Dockerfile in the 'web-app' directory
        ...
        stage('Replace Image in Container') { // Stage for replacing the running Docker container with a new one
            steps {
                script {
                    // Run the Docker container from the 'demo-micro-ui-app2:latest' image
                    sh 'docker run --restart unless-stopped -d -p 9084:9084 demo-micro-ui-app2:latest'
                }
            }
        }
    }
